...
description: Methods for validating root cause analyses with structured checklists. Verifies 5 Whys depth, execution path accuracy, fix strategy soundness, and side effects. Use when reviewing RCA reports, validating RCA methodology, checking if root cause is fundamental, verifying file:line references, assessing fix strategies before implementation, or quality-checking RCA documents to prevent shallow analyses.
license: MIT
