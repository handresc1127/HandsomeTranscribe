...
description: Methodology for performing root cause analysis on software bugs using 5 Whys technique. Drills down from symptoms to fundamental causes. Generates multiple fix strategies with risk assessment. Use when trying to identify WHY a bug occurs (not just WHERE), analyzing system failures, understanding unexpected behavior, need to find root cause not symptom, proposing fix strategies, or analyzing after verified research is complete.
license: MIT
...
metadata:
  ...
  related-skills: codebase-research, rca-verification
---
...
**Wrong Approach**: "The bug is in `auth.ts:123` where user is null"
...
|---|------|------------|----------|
| 1 | Why does it crash? | The `user` object is null when accessing `.name` | `user-service.ts:123` |
| 2 | Why is user null? | The database query returns null when user not found | `db-layer.ts:45` |
| 3 | Why does query return null? | The query is designed to return null for missing records | `db-layer.ts:40-50` |
| 4 | Why isn't null checked? | The service assumes authentication guarantees user exists | `user-service.ts:100-125` |
| 5 | Why this assumption? | **ROOT CAUSE**: Service was originally written for authenticated-only endpoints, but was reused for public endpoints without adding null checks | Design history + code reuse |

**Root Cause**: The user service was designed for authenticated contexts where users are guaranteed to exist, but was reused for public endpoints without adding defensive null checks.
...
|----------|-------------|-------------------|
| **Logic Error** | Incorrect conditions, operators, off-by-one | Wrong comparison, boundary errors |
| **State Management** | Race conditions, stale state, missing updates | Timing issues, cache problems |
| **Resource Management** | Leaks, exhaustion, improper cleanup | Memory growth, handle exhaustion |
| **Data Issues** | Invalid input, type mismatch, encoding | Parsing errors, validation gaps |
| **Integration** | API changes, version mismatch, timing | External service failures |
| **Configuration** | Wrong settings, missing values, environment | Env-specific issues |
| **Concurrency** | Deadlock, race condition, thread safety | Intermittent failures |
| **Error Handling** | Swallowed exceptions, wrong error type | Silent failures |
...
✅ **Specific**: "Wrap database call in try/catch at `db.ts:78-82`, catch `QueryError`, return `Result.err()` with user-friendly message"
...
✅ GOOD: "Cache invalidation at `cache.ts:123` only clears entries matching exact key, doesn't handle wildcard patterns per documentation at line 45-50"
